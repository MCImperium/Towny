...
public class DrawUtil {
	 ...
	 */
	public static void runOnSurface(Level world, int x1, int z1, int x2, int z2, int height, Consumer<BlockPos> locationConsumer) {
		...
		for (int z = _z1; z <= _z2; z++) {
			for (int x = _x1; x <= _x2; x++) {
				...
				for (int y = start; y <= end; y++) {
					locationConsumer.accept(new BlockPos(world, x, y, z));
				}
			}
		}
	}
	...
	/**
	 * @deprecated Deprecated as of 0.98.3.13, please use {@link #runOnSurface(Level, int, int, int, int, int, Consumer)} instead.
	 */
	@Deprecated
	public static void runOnSurface(Level world, int x1, int z1, int x2, int z2, int height, BlockPosRunnable runnable) {
		runOnSurface(world, x1, z1, x2, z2, height, (Consumer<BlockPos>) runnable::run);
	}
}
