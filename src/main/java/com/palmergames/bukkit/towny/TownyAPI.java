...
public class TownyAPI {
    ...
    /**
     * Gets the town spawn {@link BlockPos} of a {@link Player}.
     *
     * @param player {@link Player} of which you want the town spawn.
     * @return {@link BlockPos} of the town spawn or if it is not obtainable null.
     */
    @Nullable
    public BlockPos getTownSpawnBlockPos(Player player) {
    ...
    /**
     * Gets the nation spawn {@link BlockPos} of a {@link Player}.
     *
     * @param player {@link Player} of which you want the nation spawn.
     * @return {@link BlockPos} of the nation spawn or if it is not obtainable null.
     */
    @Nullable
    public BlockPos getNationSpawnBlockPos(Player player) {
		Resident resident = townyUniverse.getResident(player.getUUID());
    ...
    public Resident getResident(Player player) {
    	return getResident(player.getUUID());
    }
    ...
    public boolean isWilderness(Block block) {
        return isWilderness(block.getBlockPos());
    }
    
    /**
     * Check if the specified {@link BlockPos} is in the wilderness.
     *
     * @param location {@link BlockPos} to test widlerness for.
     * @return true if the {@link BlockPos} is in the wilderness, false otherwise.
     */
    public boolean isWilderness(BlockPos location) {
	 ...
	 * 
	 * @param location BlockPos to check for PVP.
	 * @return true if PVP is enabled or this isn't a world with Towny enabled.
	 */
	public boolean isPVP(BlockPos location) {
	 ...
	 * 
	 * @param location BlockPos to check for mobs status.
	 * @return true if Towny would let mobs spawn/exist at the given BlockPos, or if
	 *         Towny is disabled in the BlockPos's world.
	 * @since 0.98.2.4.
	 */
	public boolean areMobsEnabled(BlockPos location) {
     ...
     */
    public boolean isTownyWorld(Level world) {
     ...
     */
    @Nullable
    public TownyWorld getTownyWorld(Level world) {
    	return getTownyWorld(world.getName());
    }
    
    /**
     * Get the {@link Town} at a specific {@link BlockPos}.
     *
     * @param location {@link BlockPos} to get {@link Town} for.
     * @return {@link Town} at this location, or {@code null} for none.
     */
    @Nullable
    public Town getTown(BlockPos location) {
    ...
    /**
     * Get the name of a {@link Town} at a specific {@link BlockPos}.
     *
     * @param location {@link BlockPos} to get {@link Town} name for.
     * @return {@link String} containg the name of the {@link Town} at this location, or {@code null} for none.
     */
    @Nullable
    public String getTownName(BlockPos location) {
    	Town town = getTown(location);
    	return town != null ? town.getName() : null;
    }
    
    /**
     * Get the {@link UUID} of a {@link Town} at the specified {@link BlockPos}.
     *
     * @param location {@link BlockPos} to get {@link Town} {@link UUID} for.
     * @return {@link UUID} of any {@link Town} at this {@link BlockPos}, or {@code null} for none.
     */
    @Nullable
    public UUID getTownUUID(BlockPos location) {
    	Town town = getTown(location);
    ...
    /**
     * Get the {@link TownBlock} at a specific {@link BlockPos}.
     *
     * @param location {@link BlockPos} to get {@link TownBlock} of.
     * @return {@link TownBlock} at this {@link BlockPos}, or {@code null} for none.
     */
    @Nullable
    public TownBlock getTownBlock(BlockPos location) {
        WorldCoord worldCoord = WorldCoord.parseWorldCoord(location);
    ...
    public TownBlock getTownBlock(@NotNull Player player) {
		return WorldCoord.parseWorldCoord(player.getBlockPos()).getTownBlockOrNull();
    }
	...
	public void requestTeleport(Player player, BlockPos spawnLoc) {
		requestTeleport(player, spawnLoc, 0);
	}

    public void requestTeleport(Player player, BlockPos spawnLoc, int cooldown) {
     ...
     * 
     * @param location - BlockPos to test.
     * @return true if the location is in a NationZone.
     */
    public boolean isNationZone(BlockPos location) {
     ...
     * Recommended to use {@link TownyAPI#isWilderness(BlockPos)} prior to using this, to confirm the location is not in a town.  
     * 
     * @param location - BlockPos to test whether it is a nation zone or normal wilderness.
     * @return returns either UNCLAIMED_ZONE or NATION_ZONE
     */
    public TownBlockStatus hasNationZone(BlockPos location) {
    ...
    /**
     * Teleports the Player to the specified jail {@link BlockPos}.
     *
     * @param player   {@link Player} to be teleported to jail.
     * @param location {@link BlockPos} of the jail to be teleported to.
     ...
     */
	@Deprecated
    public void jailTeleport(final Player player, final BlockPos location) {
