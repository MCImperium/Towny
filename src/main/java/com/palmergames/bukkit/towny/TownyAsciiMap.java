...
public class TownyAsciiMap {
	...
	public static Component[] generateCompass(Player player) {
		...
		Compass.Point dir = Compass.getCompassPointForDirection(player.getBlockPos().getYaw());
	...
	public static void generateAndSend(Towny plugin, Player player, int lineHeight) {
		...
		Coord pos = Coord.parseCoord(plugin.getCache(player).getLastBlockPos());
		final Translator translator = Translator.locale(player);
		...
		TownBlock townblock = TownyAPI.getInstance().getTownBlock(plugin.getCache(player).getLastBlockPos());
