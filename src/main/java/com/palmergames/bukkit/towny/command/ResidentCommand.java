...
public class ResidentCommand extends BaseCommand implements CommandExecutor {
					...
					switch (args.length) {
						...
						case 3:
							if (args[1].equalsIgnoreCase("remove")) {
								Resident res = TownyUniverse.getInstance().getResident(((Player) sender).getUUID());
								if (res != null)
	...
	private void parseResidentCommandForConsole(final Player sender, String[] split) throws TownyException {
	...
	public void parseResidentCommand(final Player player, String[] split) {
		...
		try {
			...
			if (split.length == 0) {
				Resident res = getResidentOrThrow(player.getUUID());
				...
				if (split.length > 1)
					...
					res = getResidentOrThrow(player.getUUID());
				...
				SpawnUtil.sendToTownySpawn(player, split, getResidentOrThrow(player.getUUID()), Translatable.of("msg_err_cant_afford_tp").forLocale(player), false, false, SpawnType.RESIDENT);
	...
	private void residentToggle(Player player, String[] newSplit) throws TownyException {
		Resident resident = TownyUniverse.getInstance().getResidentOpt(player.getUUID())
	...
	public void listResidents(Player sender) {
	...
	public void residentSet(Player player, String[] split) throws TownyException {
		...
		if (split.length == 0) {
			...
			Optional<Resident> resOpt = townyUniverse.getResidentOpt(player.getUUID());
