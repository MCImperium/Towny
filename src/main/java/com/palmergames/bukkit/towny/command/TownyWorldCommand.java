...
public class TownyWorldCommand extends BaseCommand implements CommandExecutor {
	...
	public List<String> tabComplete(Player sender, String[] args, boolean showWorlds) {
	...
	public void parseWorldCommand(Player sender, String[] split) {
	...
	public void listWorlds(Player sender) {
	...
	public void worldToggle(Player sender, String[] split) throws TownyException {
		if (split.length == 0 ) {
			if (sender instanceof Player)		
			...
			if (split[0].equalsIgnoreCase("claimable")) {
				...
				TownyMessaging.sendMsg(sender, Translatable.of("msg_changed_world_setting", "Level Mob spawns", globalWorld.getName(), formatBool(globalWorld.hasWorldMobs())));
	...
	public void worldSet(Player sender, String[] split) {
