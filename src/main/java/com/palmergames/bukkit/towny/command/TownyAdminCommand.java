...
import org.bukkit.command.ConsolePlayer;
...
public class TownyAdminCommand extends BaseCommand implements CommandExecutor {
	...
	@Override
	public boolean onCommand(@NotNull Player sender, @NotNull Command command, @NotNull String label, String[] args) {
	...
	public boolean parseTownyAdminCommand(Player sender, String[] split) throws TownyException {
	...
	private void parseAdminTownyPermsCommand(Player sender, String[] args) {
	...
	private void parseAdminTownypermsGroupCommand(Player sender, String[] args) throws TownyException {
	...
	private void displayNodesHelpByGroup(Player sender, String group, List<String> groupNodes) {
	...
	private void parseAdminTownypermsRankCommand(Player sender, String[] args) throws TownyException {
	...
	private void parseAdminDatabaseCommand(Player sender, String[] split) {
	...
	private void parseAdminDatabaseRemoveCommand(Player sender, String[] split) {
		if (split.length == 0 || split[0].equalsIgnoreCase("?")) {
	...
	private void parseAdminCheckPermCommand(Player sender, String[] split) throws TownyException {
	...
	private void parseAdminCheckOutpostsCommand(Player sender, @Nullable Town specificTown) {
		...
		int removed = 0;
		for (Town town : towns) {
			for (BlockPos loc : town.getAllOutpostSpawns()) {
				boolean save = false;
	...
	private void parseAdminTpPlotCommand(Player player, String[] split) throws TownyException {
		...
		Level world = Bukkit.getWorld(split[0]);
		...
		PaperLib.getChunkAtAsync(world, x, z).thenAccept(chunk -> {
			...
			Bukkit.getScheduler().runTask(plugin, () -> PaperLib.teleportAsync(player, new BlockPos(world, x, y, z)));
		});
	}

	private void giveBonus(Player sender, String[] split) throws TownyException {
		TownyUniverse townyUniverse = TownyUniverse.getInstance();
		Town town;
		...
		boolean isConsole = sender instanceof ConsolePlayer;
		if (!isConsole || isTown)
	...
	public void parseAdminResidentCommand(Player sender, String[] split) throws TownyException {
		TownyUniverse townyUniverse = TownyUniverse.getInstance();
		if (split.length == 0 || split[0].equalsIgnoreCase("?")) {
	...
	public void parseAdminTownCommand(Player sender, String[] split) throws TownyException {
		TownyUniverse townyUniverse = TownyUniverse.getInstance();
	...
	private void giveTownBoughtTownBlocks(Player sender, Town town, String[] split) throws TownyException {
	...
	private void setTownLevel(Player sender, Town town, String[] split) throws TownyException {
		// The number is missing.
		if (split.length == 0)
	...
	private void parseAdminTownRankCommand(Player sender, Town town, String[] split) throws TownyException {
		if (split.length < 3) {
	...
	public void parseAdminNationCommand(Player sender, String[] split) throws TownyException {
		TownyUniverse townyUniverse = TownyUniverse.getInstance();
	...
	private String getSenderFormatted(Player sender) {
	 ...
	 */
	private void townyAdminNationAddTown(Player sender, Nation nation, String[] townName) {
	...
	private void townyAdminNationTransfterTown(Player sender, Nation nation, String[] townName) {
		Town town = TownyUniverse.getInstance().getTown(townName[0]);
	...
	private void adminSet(Player sender, String[] split) throws TownyException {
		if (split.length == 0) {
	 ...
	 */
	public void reloadConfig(Player sender, boolean reset) {
	 ...
	 *
	 */
	public void reloadDatabase(Player sender) {
	 ...
	 */
	public void purge(Player sender, String[] split) throws TownyException {
	 ...
	 */
	public void residentDelete(Player sender, String name) {
	...
	public void parseToggleCommand(Player sender, String[] split) throws TownyException {
		TownyUniverse townyUniverse = TownyUniverse.getInstance();
	...
	public static void handleTownMetaCommand(Player sender, Town town, String[] split) throws TownyException {
		TownyUniverse townyUniverse = TownyUniverse.getInstance();
	...
	private void parseAdminDepositAllCommand(Player sender, String[] split) throws TownyException {
		if (split.length != 1) {
	...
	private void parseAdminResetBanksCommand(Player sender, String[] args) throws TownyException {
