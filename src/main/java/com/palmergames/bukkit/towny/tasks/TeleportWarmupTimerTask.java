...
public class TeleportWarmupTimerTask extends TownyTimerTask {
	...
	public static void requestTeleport(Resident resident, BlockPos spawnLoc) {
		requestTeleport(resident, spawnLoc, 0);
	}

	public static void requestTeleport(Resident resident, BlockPos spawnLoc, int cooldown) {
		...
		try {
			...
			Towny.getPlugin().getLogger().error("Error: Null returned from teleport queue.");
			e.printStackTrace();
		}
	}
