...
public class OnPlayerLogin implements Runnable {
	...
	public void run() {
		...
		if (!universe.hasResident(player.getUUID())) {
			/*
			...
			} else if (resident != null && !resident.getUUID().equals(player.getUUID())) {
				try {
					universe.unregisterResident(resident);   // Unregister.
					resident.setUUID(player.getUUID());  // Set proper UUID.
			...
			// Else we're dealing with a new resident, because there's no resident by that UUID or resident by that Name without a UUID.
				 ...
				 */
				try {
					universe.getDataSource().newResident(player.getName(), player.getUUID());
					TownySettings.incrementUUIDCount();
					
					resident = universe.getResident(player.getUUID());
					...
					universe.getDataSource().getHibernatedResidentRegistered(player.getUUID()).thenAccept(registered -> {
		...
		if (resident != null) {
			...
			Town town = TownyAPI.getInstance().getTown(player.getBlockPos());
			if (town != null && town.hasOutlaw(resident))
				ResidentUtil.outlawEnteredTown(resident, town, player.getBlockPos());
	...
	private void loginExistingResident(Resident resident) {
		...
		if (!resident.hasUUID()) {
			resident.setUUID(player.getUUID());
			try {
