...
public class CombatUtil {
	 ...
	 * permissions to damage passive animals and villagers while in a town.
	 * 
	 * @param world - Level in which DamageCall was issued
	...
	private static boolean preventDamageCall(TownyWorld world, Entity attackingEntity, Entity defendingEntity, Player attackingPlayer, Player defendingPlayer, DamageCause cause) {
		...
		TownBlock defenderTB = TownyAPI.getInstance().getTownBlock(defendingEntity.getBlockPos());
		TownBlock attackerTB = TownyAPI.getInstance().getTownBlock(attackingEntity.getBlockPos());
		/*
		...
		if (attackingPlayer != null) {
			...
			if (defendingPlayer != null) {
				 ...
				 */
				TownyPlayerDamagePlayerEvent event = new TownyPlayerDamagePlayerEvent(defendingPlayer.getBlockPos(), defendingPlayer, cause, defenderTB, cancelled, attackingPlayer);
			 ...
			 */
				...
				if (defenderTB != null) {
					...
					if (defendingEntity instanceof Wolf wolf) {
						if (!isOwner(wolf, attackingPlayer) && !isTargetingPlayer(wolf, attackingPlayer)) {
							if (EntityTypeUtil.isProtectedEntity(defendingEntity))
								return !(defenderTB.getPermissions().pvp || TownyActionEventExecutor.canDestroy(attackingPlayer, wolf.getBlockPos(), Material.STONE));
						} else
					...
					if (defenderTB.getType() == TownBlockType.FARM && TownySettings.getFarmAnimals().contains(defendingEntity.getType().toString()))
						return !TownyActionEventExecutor.canDestroy(attackingPlayer, defendingEntity.getBlockPos(), Material.WHEAT);

					/*
					 * Config's protected entities: Animals,WaterMob,NPC,Snowman,ArmorStand,Villager
					 */
					if (EntityTypeUtil.isProtectedEntity(defendingEntity))
						return !TownyActionEventExecutor.canDestroy(attackingPlayer, defendingEntity.getBlockPos(), Material.DIRT);
				}
				...
				if (material != null) {
					...
					return !TownyActionEventExecutor.canDestroy(attackingPlayer, defendingEntity.getBlockPos(), material);
				}
			}
		 ...
		 */
			...
			if (defendingPlayer != null) {
				...
				if (attackingEntity instanceof LightningStrike 
					&& world.hasTridentStrike(attackingEntity.getUUID())
				...
				*/
				if (attackingEntity instanceof Wolf wolf && EntityTypeUtil.isInstanceOfAny(TownySettings.getProtectedEntityTypes(), defendingEntity)) {
					if (isATamedWolfWithAOnlinePlayer(wolf)) {
						...
						return !PlayerCacheUtil.getCachePermission(owner, defendingEntity.getBlockPos(), Material.AIR, ActionType.DESTROY);
					} else {
						wolf.setTarget(null);
						wolf.setAngry(false);
						return true;
	...
	private static boolean preventJailedPVP(Player defendingPlayer, Player attackingPlayer) {
		if (TownySettings.doJailPlotsPreventPVP()) {
			Resident defendingResident = TownyAPI.getInstance().getResident(defendingPlayer.getUUID());
			Resident attackingResident = TownyAPI.getInstance().getResident(attackingPlayer.getUUID());
	 ...
	 * @param world - Level to check if TownBlock is NULL
	...
	private static boolean isPvP(@NotNull TownBlock townBlock) {
		...
		// Checks PVP perm: 1. Plot PVP, 2. Town PVP, 3. Level Force PVP 
	...
	private static boolean isOwner(Wolf wolf, Player attackingPlayer) {
		return wolf.getOwner() instanceof HumanEntity owner && owner.getUUID().equals(attackingPlayer.getUUID());
	}
	...
	private static boolean isATamedWolfWithAOnlinePlayer(Wolf wolf) {
		return wolf.getOwner() instanceof HumanEntity owner && Bukkit.getPlayer(owner.getUUID()) != null;
	}
	...
	public static boolean preventDispenserDamage(Block dispenser, Entity entity, DamageCause cause) {
		...
		return BukkitTools.isEventCancelled(new TownyDispenserDamageEntityEvent(entity.getBlockPos(), entity, cause, defenderTB, preventDamage, dispenser));
	}
}
