...
public class SpawnUtil {
	...
	public static void outlawTeleport(Town town, Resident outlaw) {
		...
		PaperLib.getBedSpawnBlockPosAsync(outlawedPlayer, true).thenAccept(bed -> {
			BlockPos spawnBlockPos = town.getWorld().getSpawnBlockPos();
			if (!TownySettings.getOutlawTeleportWorld().equals(""))
				spawnBlockPos = Objects.requireNonNull(Bukkit.getWorld(TownySettings.getOutlawTeleportWorld())).getSpawnBlockPos();
			
			if (bed != null && TownyAPI.getInstance().getTown(bed) != town)
				spawnBlockPos = bed;
			
			if (outlaw.hasTown() && TownyAPI.getInstance().getTownSpawnBlockPos(outlawedPlayer) != null)
				spawnBlockPos = TownyAPI.getInstance().getTownSpawnBlockPos(outlawedPlayer);
			
			TownyMessaging.sendMsg(outlaw, Translatable.of("msg_outlaw_kicked", town));
			initiatePluginTeleport(outlaw, spawnBlockPos, true);
		});		
	}
	...
	public static void jailAwayTeleport(Resident jailed) {
		initiatePluginTeleport(jailed, getIdealBlockPos(jailed), false);
	}
	 ...
	 * @return spawnLoc BlockPos the player will spawn to.
	 * @throws TownyException thrown when the eventual spawn location is invalid or
	 *                        the player is outlawed at that location.
	 */
	private static CompletableFuture<BlockPos> getSpawnLoc(Player player, Town town, Nation nation, SpawnType spawnType, boolean outpost, String[] split) throws TownyException {
		return switch (spawnType) {
			case RESIDENT:
				if (TownySettings.getBedUse()) {
					yield PaperLib.getBedSpawnBlockPosAsync(player, true).thenApply(bedLoc -> {
						if (bedLoc != null)
							return bedLoc;
						else if (town != null && town.hasSpawn())
							return town.getSpawnOrNull();
						else
							return plugin.getCache(player).getLastBlockPos().getWorld().getSpawnBlockPos();
					});
				} else if (town != null && town.hasSpawn())
					yield CompletableFuture.completedFuture(town.getSpawnOrNull());
				else
					yield CompletableFuture.completedFuture(plugin.getCache(player).getLastBlockPos().getWorld().getSpawnBlockPos());
			case TOWN:
				if (outpost)
					yield CompletableFuture.completedFuture(getOutpostSpawnBlockPos(town, split));
				else
	 ...
	 * @return BlockPos of the town's outpost spawn.
	 * @throws TownyException thrown when there are no outposts, or the outpost
	 *                        limit was capped.
	 */
	private static BlockPos getOutpostSpawnBlockPos(Town town, String[] split) throws TownyException {
	...
	private static void testDisallowedZones(Player player, Resident resident, SpawnType spawnType, List<String> disallowedZones) throws TownyException {
		if (!disallowedZones.isEmpty()) {
			Town townAtPlayerLoc = TownyAPI.getInstance().getTown(player.getBlockPos());
	 ...
	 * @param spawnType SpawnType (RESIDENT, TOWN, NATION).
	 * @param spawnLoc  BlockPos being spawned to.
	 * @throws TownyException when the event is cancelled.
	 */
	private static void sendSpawnEvent(Player player, SpawnType spawnType, BlockPos spawnLoc) throws TownyException {
	 ...
	 * @param spawnLoc  BlockPos that the player will spawn at.
	 * @return SpawnEvent to be called.
	 */
	private static SpawnEvent getSpawnEvent(Player player, SpawnType spawnType, BlockPos spawnLoc) {
		return switch(spawnType) {
		case RESIDENT -> new ResidentSpawnEvent(player, player.getBlockPos(), spawnLoc);
		case TOWN -> new TownSpawnEvent(player, player.getBlockPos(), spawnLoc);
		case NATION -> new NationSpawnEvent(player, player.getBlockPos(), spawnLoc);
		};
	}
	 ...
	 * @param spawnLoc BlockPos being spawned to.
	 */
	private static void initiateSpawn(Player player, BlockPos spawnLoc, int cooldown) {
	 ...
	 * @param spawnLoc   BlockPos that the player is being teleported to.
	 ...
	 *                   cost confirmation.
	 */
	private static void initiateCostedSpawn(Player player, Resident resident, BlockPos spawnLoc, double travelCost, Account payee, String paymentMsg, boolean ignoreWarn, int cooldown) {
	 ...
	 * @param paymentMsg Message being left in the Towny money.csv log.
	 */
	private static void payAndThenSpawn(Player player, Resident resident, BlockPos spawnLoc, double travelCost, Account payee, String paymentMsg, int cooldown) {
	 ...
	 */
	private static CompletableFuture<BlockPos> getIdealBlockPos(Resident resident) {
		Town town = resident.getTownOrNull();
		BlockPos loc = resident.getPlayer().getWorld().getSpawnBlockPos();

		if (town != null && town.hasSpawn())
			loc = town.getSpawnOrNull();

		BlockPos finalLoc = loc;
		return PaperLib.getBedSpawnBlockPosAsync(resident.getPlayer(), true).thenApply(bed -> bed == null ? finalLoc : bed);
	}
	 ...
	 * @param loc          BlockPos the resident will go to.
	 * @param ignoreWarmup True if they will teleport instantly, false to use
	 *                     TeleportWarmupTime from the config.
	 */
	private static void initiatePluginTeleport(Resident resident, BlockPos loc, boolean ignoreWarmup) {
	...
	private static void initiatePluginTeleport(Resident resident, CompletableFuture<BlockPos> loc, boolean ignoreWarmup) {
