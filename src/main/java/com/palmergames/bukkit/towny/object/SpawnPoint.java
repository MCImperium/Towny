...
public class SpawnPoint {
	...
	private final SpawnPointBlockPos spawnBlockPos;
	
	private static final ArrayList<RingCoord> RING_PATTERN = createRing();
	
	public SpawnPoint(BlockPos loc, SpawnPointType type) {
		this.location = loc;
		this.type = type;
		this.wc = WorldCoord.parseWorldCoord(loc);
		this.spawnBlockPos = new SpawnPointBlockPos(loc);
	}
	...
	public BlockPos getBukkitBlockPos() {
		return location;
	}
	
	public SpawnPointBlockPos getSpawnPointBlockPos() {
		return spawnBlockPos;
	}

	public void drawParticle() {
		BlockPos origin = centreBlockPos(location);
		int i = 0;

		for (RingCoord ringPosition : RING_PATTERN) {
			BlockPos point = origin.clone().add(ringPosition.getX(), 0.0d, ringPosition.getZ());
	...
	private BlockPos centreBlockPos(BlockPos loc) {
