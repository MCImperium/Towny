...
public class EditGUI extends PermissionGUI {
	...
	public EditGUI(Resident res, GuiContainer inv, String name, TownBlock townBlock, Resident selectedResident) {
	...
	public void saveChanges() {
		...
		for (int i = 0; i < 4; i++) {
			Item item = getInventory().getItem(PermissionGUIUtil.getWoolSlots()[i]).getItem();
			if (item.equals(Items.LIME_WOOL)) {
				newTypes[i] = SetPermissionType.SET;
			} else if (item.equals(Items.RED_WOOL)) {
