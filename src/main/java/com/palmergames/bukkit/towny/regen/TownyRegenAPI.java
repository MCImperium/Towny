...
public class TownyRegenAPI {
	...
	private static final Hashtable<BlockLoc, ProtectionRegenTask> protectionRegenTasks = new Hashtable<>();
...
//			Chunk chunk = world.getChunkAt(player.getBlockPos());
...
//							holder.setBlockPos(state.getBlockPos());
//							snapshot[x][y][z] = holder;
//							
//						} else {
//						
//							snapshot[x][y][z] = new BlockObject(BukkitTools.getTypeId(state), BukkitTools.getDataData(state), state.getBlockPos());
//									
//						}
//						
//					}
...
//		Level world = key.getBlockPos().getWorld();
//		Chunk chunk = key.getBlockPos().getChunk();
//		
//		int maxHeight = world.getMaxHeight();
//		
//		for (int x = 0; x < 16; x++) {
...
//	}
	...
	public static void deleteMaterialsFromWorldCoord(WorldCoord coord, Collection<Material> collection) {
		...
		Level world = coord.getBukkitWorld();
		if (world == null)
			return;
	...
	public static boolean beginProtectionRegenTask(Block block, int count, TownyWorld world, Event event) {
		...
		if (!hasProtectionRegenTask(new BlockLoc(block.getBlockPos()))) {
	 ...
	 * 
	 * @param blockLoc - BlockPos of the block
	 * @return true if a task exists
	 */
	public static boolean hasProtectionRegenTask(BlockLoc blockLoc) {

		return protectionRegenTasks.containsKey(blockLoc);
	 ...
	 */
	public static ProtectionRegenTask GetProtectionRegenTask(BlockLoc blockLoc) {

		if (protectionRegenTasks.containsKey(blockLoc))
			return protectionRegenTasks.get(blockLoc);
	...
	public static void addProtectionRegenTask(ProtectionRegenTask task) {
		...
		protectionRegenTasks.put(task.getBlockBlockPos(), task);
	}
	...
	public static void removeProtectionRegenTask(ProtectionRegenTask task) {
		...
		protectionRegenTasks.remove(task.getBlockBlockPos());
	...
	public static void cancelProtectionRegenTasks() {
		...
		try {
			if (Class.forName("org.spigotmc.WatchdogThread").isInstance(Thread.currentThread())) {
				Towny.getPlugin().getLogger().error("Detected a watchdog crash, ongoing protection revert tasks will not be finished.");
