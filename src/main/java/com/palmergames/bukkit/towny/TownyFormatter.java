...
import com.palmergames.bukkit.towny.object.SpawnBlockPos;
...
public class TownyFormatter {
	 ...
	 * @return StatusScreen containing the results.
	 */
	public static StatusScreen getStatus(Resident resident, Player sender) {
	 ...
	 * @param sender Player who will be sent the status.   
	 * @return StatusScreen containing the results.
	 */
	public static StatusScreen getStatus(Town town, Player sender) {
	 ...
	 * @return StatusScreen containing the results.
	 */
	public static StatusScreen getStatus(Nation nation, Player sender) {
	...
	/**
	 * Gets the status screen for a Level.
	 * 
	 * @param world the world to check.
	 * @param sender Player who will be sent the status.
	 * @return StatusScreen containing the results.
	 */
	public static StatusScreen getStatus(TownyWorld world, Player sender) {
	...
	/**
	 * Returns the 2nd line of the Level StatusScreen.
	 * @param world TownyWorld for which to get the StatusScreen. 
	 * @param translator Translator used in language selection.
	 * @return Formatted 2nd line of the Level StatusScreen.
	 */
	 ...
	 */
	private static boolean playerIsOnlineAndVisible(String name, Player sender) {
	...
	private static String formatWebUrl(SpawnBlockPos spawnBlockPos) {
		String webUrl = "";
		if (TownySettings.isUsingWebMapStatusScreens() && spawnBlockPos.hasSpawn() && !TownySettings.getWebMapUrl().isEmpty())
			webUrl = TownySettings.getWebMapUrl()
				.replaceAll("\\{world}", spawnBlockPos.getSpawnOrNull().getWorld().getName())
				.replaceAll("\\{x}", "" + spawnBlockPos.getSpawnOrNull().getBlockX())
				.replaceAll("\\{z}", "" + spawnBlockPos.getSpawnOrNull().getBlockZ());
