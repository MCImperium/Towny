...
public abstract class TownyDataSource {
	...
	public boolean loadTowns() {
		...
		for (Town town : universe.getTowns())
			if (!loadTown(town)) {
				plugin.getLogger().error("Loading Error: Could not read town data '" + town.getName() + "'.");
				return false;
			}
		return true;
	}
	...
	public boolean loadNations() {
		...
		for (Nation nation : universe.getNations())
			if (!loadNation(nation)) {
				plugin.getLogger().error("Loading Error: Could not read nation data '" + nation.getName() + "'.");
				return false;
			}
		return true;
	}
	...
	public boolean loadWorlds() {
		...
		for (TownyWorld world : universe.getTownyWorlds())
			if (!loadWorld(world)) {
				plugin.getLogger().error("Loading Error: Could not read world data '" + world.getName() + "'.");
				return false;
			}
		return true;
	}
	...
	public boolean loadJails() {
		...
		for (Jail jail : universe.getJails()) {
			if (!loadJail(jail)) {
				plugin.getLogger().error("Loading Error: Could not read jail data '" + jail.getUUID() + "'.");
				return false;
			}
		}
		return true;
	...
	public boolean loadPlotGroups() {
		...
		for (PlotGroup group : universe.getGroups()) {
			if (!loadPlotGroup(group)) {
				plugin.getLogger().error("Loading Error: Could not read PlotGroup data: '" + group.getUUID() + "'.");
				return false;
			}
		}
		return true;
