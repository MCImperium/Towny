...
public class TownyBlockListener implements Listener {
	...
	public void onBlockBreak(BlockBreakEvent event) {
		...
		event.setCancelled(!TownyActionEventExecutor.canDestroy(event.getPlayer(), block.getBlockPos(), block.getType()));
	}
	...
	public void onBlockPlace(BlockPlaceEvent event) {
		...
		//Cancel based on whether this is allowed using the PlayerCache and then a cancellable event.
		if (!TownyActionEventExecutor.canBuild(event.getPlayer(), block.getBlockPos(), block.getType())) {
	...
	public void onBlockCanBuild(BlockCanBuildEvent event) {
		...
		if (event.getMaterial().name().equals("POWDER_SNOW")) {
			...
			if (!TownyActionEventExecutor.canBuild(event.getPlayer(), event.getBlock().getBlockPos(), event.getBlock().getType()))
	...
	private void testDoubleChest(Player player, Block block) {
		...
		if (!blocksToUpdate.isEmpty())  // Update the player with the new chest appearances.
			for (Block b : blocksToUpdate)
				player.sendBlockChange(b.getBlockPos(), b.getBlockData());
	}
	...
	public void onCreateExplosion(BlockExplodeEvent event) {
		 ...
		 */
		if (material == Material.AIR && townyWorld.hasBedExplosionAtBlock(event.getBlock().getBlockPos()))
			material = townyWorld.getBedExplosionMaterial(event.getBlock().getBlockPos());
	...
	public void onFrostWalkerFreezeWater(EntityBlockFormEvent event) {
		...
		if (event.getEntity() instanceof Player player) {
			//Cancel based on whether this is allowed using the PlayerCache and then a cancellable event.
			event.setCancelled(!TownyActionEventExecutor.canBuild(player, event.getBlock().getBlockPos(), event.getBlock().getType()));
		}
	}
