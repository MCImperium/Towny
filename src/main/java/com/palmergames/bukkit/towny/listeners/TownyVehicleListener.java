...
public class TownyVehicleListener implements Listener {
	...
	public void onVehicleDamage(VehicleDamageEvent event) {
		...
		if (event.getAttacker() == null) {  // Probably a respawn anchor or a TNT minecart or TNT lit by redstone.
			event.setCancelled(!TownyActionEventExecutor.canExplosionDamageEntities(event.getVehicle().getBlockPos(), event.getVehicle(), DamageCause.ENTITY_EXPLOSION));
			return;
		}
		...
		if (event.getAttacker() instanceof Player player) {
			...
			if (vehicle != null) {
				...
				event.setCancelled(!TownyActionEventExecutor.canDestroy(player, event.getVehicle().getBlockPos(), vehicle));
			}
		} else {
			if (EntityTypeUtil.isExplosive(event.getAttacker().getType()) && !TownyActionEventExecutor.canExplosionDamageEntities(event.getVehicle().getBlockPos(), event.getVehicle(), DamageCause.ENTITY_EXPLOSION))
				event.setCancelled(true);
		}
	}
	...
	public void onVehicleCollide(VehicleBlockCollisionEvent event) {
		...
		if (event.getVehicle() instanceof Minecart && ItemLists.MINECART_KILLERS.contains(event.getBlock().getType())) {
			...
			event.getBlock().getWorld().dropItemNaturally(event.getVehicle().getBlockPos(), new ItemStack(EntityTypeUtil.parseEntityToMaterial(event.getVehicle().getType())));
		}
	}
	...
	public void onVehicleEnter(VehicleEnterEvent event) {
		...
		if (event.getEntered() instanceof Player player) {
			...
			if (vehicle != null) {
				//Make decision on whether this is allowed using the PlayerCache and then a cancellable event.
				if (TownySettings.isSwitchMaterial(vehicle, event.getVehicle().getBlockPos()))
					event.setCancelled(!TownyActionEventExecutor.canSwitch(player, event.getVehicle().getBlockPos(), vehicle));
			}
		}	
	}
}
