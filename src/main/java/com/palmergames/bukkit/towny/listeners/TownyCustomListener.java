...
public class TownyCustomListener implements Listener {
	...
	public void onBedExplodeEvent(BedExplodeEvent event) {
		TownyWorld world = TownyAPI.getInstance().getTownyWorld(event.getBlockPos().getWorld().getName());
		world.addBedExplosionAtBlock(event.getBlockPos(), event.getMaterial());
		if (event.getBlockPos2() != null);
			world.addBedExplosionAtBlock(event.getBlockPos2(), event.getMaterial());
		final TownyWorld finalWorld = world;
		Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, new Runnable() {
            @Override
            public void run() {
                finalWorld.removeBedExplosionAtBlock(event.getBlockPos());
                finalWorld.removeBedExplosionAtBlock(event.getBlockPos2());
            }
        }, 20L);
	}
