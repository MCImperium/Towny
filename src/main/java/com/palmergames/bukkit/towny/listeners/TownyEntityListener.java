...
import com.palmergames.bukkit.towny.regen.block.BlockLoc;
...
public class TownyEntityListener implements Listener {
	...
	public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
		...
		if (event.getCause() == DamageCause.ENTITY_EXPLOSION && !(attacker instanceof Projectile)) {
			...
			if (EntityTypeUtil.isInstanceOfAny(TownySettings.getProtectedEntityTypes(), defender)
				...
				&& !TownyActionEventExecutor.canExplosionDamageEntities(event.getEntity().getBlockPos(), event.getEntity(), event.getCause()))
			...
			if (defender instanceof Player && EntityTypeUtil.isPVPExplosive(attacker.getType()))
				cancelExplosiveDamage = CombatUtil.preventPvP(TownyAPI.getInstance().getTownyWorld(defender.getWorld().getName()), TownyAPI.getInstance().getTownBlock(defender.getBlockPos()));
	...
	public void onEntityDeath(EntityDeathEvent event) {
		...
		if (!TownyAPI.getInstance().isTownyWorld(entity.getWorld()) || TownyAPI.getInstance().isWilderness(entity.getBlockPos()))
			return;
		
		if (entity instanceof Monster)
			if (TownyAPI.getInstance().getTownBlock(entity.getBlockPos()).getType() == TownBlockType.ARENA)
	...
	public void onEntityTakesBlockExplosionDamage(EntityDamageEvent event) {
		...
		if ((event.getCause() == DamageCause.BLOCK_EXPLOSION || event.getCause() == DamageCause.LIGHTNING) && !TownyActionEventExecutor.canExplosionDamageEntities(event.getEntity().getBlockPos(), event.getEntity(), event.getCause())) {
			event.setDamage(0);
			event.setCancelled(true);
		}
	}
	...
	public void onDragonFireBallCloudDamage(AreaEffectCloudApplyEvent event) {
		...
		TownBlock townBlock = TownyAPI.getInstance().getTownBlock(event.getEntity().getBlockPos());
	...
	public void onLingeringPotionSplashEvent(LingeringPotionSplashEvent event) {
		...
		BlockPos loc = potion.getBlockPos();		
		...
		for(double x = loc.getX() - radius; x < loc.getX() + radius; x++ ) {
			for(double z = loc.getZ() - radius; z < loc.getZ() + radius; z++ ) {
				BlockPos loc2 = new BlockPos(potion.getWorld(), x, loc.getY(), z);
		...
		for (Block block : blocks) {
			...
			if (!TownyAPI.getInstance().isWilderness(block.getBlockPos()) 
					&& CombatUtil.preventPvP(townyWorld, TownyAPI.getInstance().getTownBlock(block.getBlockPos()))) {
				event.setCancelled(true);
				break;
			}			
		}	
	...
	public void onCreatureSpawn(CreatureSpawnEvent event) {
		...
		if (event.getEntity() != null) {
			...
			BlockPos loc = event.getBlockPos();
			TownyWorld townyWorld = TownyAPI.getInstance().getTownyWorld(loc.getWorld().getName());
	...
	public void onEntityInteract(EntityInteractEvent event) {
		...
		if (passengers != null) {
			...
			for (Entity passenger : passengers) {
				...
				if (TownySettings.isSwitchMaterial(block.getType(), block.getBlockPos())) {
					//Make decision on whether this is allowed using the PlayerCache and then a cancellable event.
					event.setCancelled(!TownyActionEventExecutor.canSwitch(player, block.getBlockPos(), block.getType()));
					return;
				}
			}
		}
	...
	public void onEntityChangeBlockEvent(EntityChangeBlockEvent event) {
		...
		if (event.getBlock().getType().equals(Material.FARMLAND)) {
			...
			if (event.getEntity() instanceof Player player) {
				event.setCancelled(TownySettings.isPlayerCropTramplePrevented() || !TownyActionEventExecutor.canDestroy(player, event.getBlock().getBlockPos(), Material.FARMLAND));
				return;
			}
		}
		...
		switch (event.getEntity().getType()) {
			...
			case SPLASH_POTION -> {
				if (event.getBlock().getType() != Material.CAMPFIRE && ((ThrownPotion) event.getEntity()).getShooter() instanceof Player)
					return;
				event.setCancelled(!TownyActionEventExecutor.canDestroy((Player) ((ThrownPotion) event.getEntity()).getShooter(), event.getBlock().getBlockPos(), Material.CAMPFIRE));
			}
	...
	public void onEntityExplode(EntityExplodeEvent event) {
		...
		if (townyWorld.isUsingPlotManagementWildEntityRevert() && entity != null && townyWorld.isProtectingExplosionEntity(entity)) {
			...
			for (Block block : event.blockList()) {
				...
				if (TownyRegenAPI.hasProtectionRegenTask(new BlockLoc(block.getBlockPos())))
					continue;
				count++;
	...
	public void onHangingBreak(HangingBreakEvent event) {
		...
		if (event.getCause().equals(RemoveCause.PHYSICS) && ItemLists.HANGING_ENTITIES.contains(hanging.getType().name())) {
			BlockPos loc = hanging.getBlockPos().add(hanging.getFacing().getOppositeFace().getDirection());
			if (TownyRegenAPI.hasProtectionRegenTask(new BlockLoc(loc))) {
				event.setCancelled(true);
				return;
			}
		}
		...
		if (event.getCause().equals(RemoveCause.PHYSICS) && ItemLists.ITEM_FRAMES.contains(hanging.getType().name())) {
			Block block = hanging.getBlockPos().add(hanging.getFacing().getOppositeFace().getDirection()).getBlock();
		...
		if (event instanceof HangingBreakByEntityEvent evt) {
			...
			if (remover instanceof Player player) {
				...
				//Make decision on whether this is allowed using the PlayerCache and then a cancellable event.
				event.setCancelled(!TownyActionEventExecutor.canDestroy(player, hanging.getBlockPos(), mat));
			} else if (remover instanceof Entity) {
				/*
				 * Probably a skeleton, cancel the break if it is in a town.
				 */
				if (!TownyAPI.getInstance().isWilderness(hanging.getBlockPos()))
					event.setCancelled(true);
			}
		
			if (event.getCause() == RemoveCause.EXPLOSION) {
				// Explosions are blocked in this plot
				if (!TownyActionEventExecutor.canExplosionDamageEntities(hanging.getBlockPos(), event.getEntity(), DamageCause.ENTITY_EXPLOSION)) {
					event.setCancelled(true);
				// Explosions are enabled, must check if in the wilderness and if we have explrevert in that world
				} else {
					TownBlock tb = TownyAPI.getInstance().getTownBlock(hanging.getBlockPos());
		 ...
		 */
			...
			if (event.getCause() == RemoveCause.EXPLOSION) {
				if (!TownyActionEventExecutor.canExplosionDamageEntities(event.getEntity().getBlockPos(), event.getEntity(), DamageCause.BLOCK_EXPLOSION))
					event.setCancelled(true);
			}
		}
	...
	public void onHangingPlace(HangingPlaceEvent event) {
		...
		//Make decision on whether this is allowed using the PlayerCache and then a cancellable event.
		event.setCancelled(!TownyActionEventExecutor.canBuild(event.getPlayer(), event.getEntity().getBlockPos(), mat));
	}
	...
	public void onPigHitByLightning(PigZapEvent event) {
		...
		if (!TownyActionEventExecutor.canExplosionDamageEntities(event.getEntity().getBlockPos(), event.getEntity(), DamageCause.LIGHTNING))
			event.setCancelled(true);
	}
	...
	public void onProjectileHitEventButtonOrPlate(ProjectileHitEvent event) {
		...
		if (ItemLists.PROJECTILE_TRIGGERED_REDSTONE.contains(material) && TownySettings.isSwitchMaterial(material, block.getBlockPos())) {
			//Make decision on whether this is allowed using the PlayerCache and then a cancellable event.
			if (!TownyActionEventExecutor.canSwitch((Player) event.getEntity().getShooter(), block.getBlockPos(), material)) {
				/*
	...
	public void onProjectileHitEventTarget(ProjectileHitEvent event) {
		...
		if (event.getHitBlock().getType() == Material.TARGET && TownySettings.isSwitchMaterial(Material.TARGET, event.getHitBlock().getBlockPos())) {
			//Make decision on whether this is allowed using the PlayerCache and then a cancellable event.
			if (!TownyActionEventExecutor.canSwitch((Player) event.getEntity().getShooter(), event.getHitBlock().getBlockPos(), Material.TARGET)) {
	...
	public void onProjectileHitEventChorusFlower(ProjectileHitEvent event) {
		...
		if (event.getHitBlock().getType() == Material.CHORUS_FLOWER) {
			//Make decision on whether this is allowed using the PlayerCache and then a cancellable event.
			if (!TownyActionEventExecutor.canDestroy((Player) event.getEntity().getShooter(), event.getHitBlock().getBlockPos(), Material.CHORUS_FLOWER)) {
	...
	public void onDoorBreak(EntityBreakDoorEvent event) {
		if (TownyAPI.getInstance().isTownyWorld(event.getBlock().getWorld()) && !TownyAPI.getInstance().isWilderness(event.getBlock().getBlockPos()))
			event.setCancelled(true);
	}
}