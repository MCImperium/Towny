...
import org.bukkit.Level.Environment;
...
public class TownyPlayerListener implements Listener {
	...
	private void sendSafeModeMessage(Player player) {
		try {
			...
			player.sendMessage(ChatFormatting.RED + "[Towny] [Error] Towny is locked in Safe Mode due to an error! " + msg);
		}
	}
	...
	public void onPlayerQuit(PlayerQuitEvent event) {
		...
		Resident resident = TownyUniverse.getInstance().getResident(event.getPlayer().getUUID());
	...
	public void onPlayerRespawn(PlayerRespawnEvent event) {
		...
		BlockPos respawn;
		respawn = TownyAPI.getInstance().getTownSpawnBlockPos(player);

		// Towny might be prioritizing bed spawns over town spawns.
		if (TownySettings.getBedUse()) { 
			BlockPos bed = player.getBedSpawnBlockPos();
			if (bed != null)
				respawn = bed;
		}
		...
		event.setRespawnBlockPos(respawn);
	...
	public void onPlayerJailRespawn(PlayerRespawnEvent event) {
		...
		if (resident != null && resident.isJailed())
			event.setRespawnBlockPos(resident.getJailSpawn());
	...
	public void onPlayerBucketEmpty(PlayerBucketEmptyEvent event) {
		...
		event.setCancelled(!TownyActionEventExecutor.canBuild(event.getPlayer(), event.getBlock().getBlockPos(), event.getBucket()));
	}
	...
	public void onPlayerBucketFill(PlayerBucketFillEvent event) {
		...
		event.setCancelled(!TownyActionEventExecutor.canDestroy(event.getPlayer(), event.getBlockClicked().getBlockPos(), event.getBlockClicked().getType()));
	...
	*/
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		...
		if (event.hasItem()) {
			...
			if (clickedBlock != null)
				loc = clickedBlock.getBlockPos();
			else 
				loc = player.getBlockPos();
			/*
			 * Test item_use. 
			 */
			...
			if (clickedBlock != null) {
				...
				if (item == Material.ARMOR_STAND || item == Material.END_CRYSTAL) 
					event.setCancelled(!TownyActionEventExecutor.canBuild(player, clickedBlock.getRelative(event.getBlockFace()).getBlockPos(), item));
		...
		if (clickedBlock != null) {
			 ...
			 * Test switch use.
			 */
			if (TownySettings.isSwitchMaterial(clickedMat, clickedBlock.getBlockPos())) {
				//Make decision on whether this is allowed using the PlayerCache and then a cancellable event.
				event.setCancelled(!TownyActionEventExecutor.canSwitch(player, clickedBlock.getBlockPos(), clickedMat));
				return;
			}
			/*
			...
			if (ItemLists.POTTED_PLANTS.contains(clickedMat) ||
				...
				//Make decision on whether this is allowed using the PlayerCache and then a cancellable event.
				event.setCancelled(!TownyActionEventExecutor.canDestroy(player, clickedBlock.getBlockPos(), clickedMat));
				return;
			}
		}
	}
	...
	public void onPlayerBlowsUpBedOrRespawnAnchor(PlayerInteractEvent event) {
		...
		Player player = event.getPlayer();
		if (event.hasBlock()) {
			BlockPos blockLoc = block.getBlockPos();
			/*
			...
			if (Tag.BEDS.isTagged(block.getType()) && player.getWorld().getEnvironment().equals(Environment.NETHER)) {
				...
				BukkitTools.fireEvent(new BedExplodeEvent(player, blockLoc, block.getRelative(bed.getFacing()).getBlockPos(), block.getType()));
				return;
			}
	...
	*/	
	...
	public void onPlayerInteractWithArmourStand(PlayerArmorStandManipulateEvent event) {
		...
		//Make decision on whether this is allowed using the PlayerCache and then a cancellable event.
		event.setCancelled(!TownyActionEventExecutor.canDestroy(event.getPlayer(), event.getRightClicked().getBlockPos(), Material.ARMOR_STAND));
	}
		...
		if (event.getRightClicked() != null) {
			...
			if (mat != null) {
				...
				if (actionType == ActionType.DESTROY) {
					//Make decision on whether this is allowed using the PlayerCache and then a cancellable event.
					event.setCancelled(!TownyActionEventExecutor.canDestroy(player, event.getRightClicked().getBlockPos(), mat));
					return;
				}
				// Material has been supplied in place of an entity, run Switch Tests.
				if (TownySettings.isSwitchMaterial(mat, event.getRightClicked().getBlockPos()) && actionType == ActionType.SWITCH) {
					//Make decision on whether this is allowed using the PlayerCache and then a cancellable event.
					event.setCancelled(!TownyActionEventExecutor.canSwitch(player, event.getRightClicked().getBlockPos(), mat));
					return;
				} 
			}
			...
			if (item != null) {
				 ...
				 */
				if (event.getRightClicked().getType().equals(EntityType.SHEEP) && item == Material.SHEARS && !TownyAPI.getInstance().isWilderness(event.getRightClicked().getBlockPos())) {
					//Make decision on whether this is allowed using the PlayerCache and then a cancellable event.
					event.setCancelled(!TownyActionEventExecutor.canDestroy(player, event.getRightClicked().getBlockPos(), item));
					return;
				}
				 ...
				 * Item_use protection.
				 */
				if (TownySettings.isItemUseMaterial(item, event.getRightClicked().getBlockPos())) {
					//Make decision on whether this is allowed using the PlayerCache and then a cancellable event.
					event.setCancelled(!TownyActionEventExecutor.canItemuse(player, event.getRightClicked().getBlockPos(), item));
					return;
				}
			}
		}
	...
	public void onPlayerFishEvent(PlayerFishEvent event) {
		...
		if (event.getState().equals(PlayerFishEvent.State.CAUGHT_ENTITY)) {
			...
			if (caught.getType().equals(EntityType.PLAYER)) {
				...
				TownBlock tb = TownyAPI.getInstance().getTownBlock(event.getCaught().getBlockPos());
				test = !CombatUtil.preventPvP(world, tb);
			// Non-player catches are tested for destroy permissions.
			} else {
				//Make decision on whether this is allowed using the PlayerCache and then a cancellable event.
				test = TownyActionEventExecutor.canDestroy(player, caught.getBlockPos(), Material.GRASS);
			}
			if (!test) {
				event.setCancelled(true);
	...
	*/
	public void onPlayerMoveChunk(Player player, WorldCoord from, WorldCoord to, BlockPos fromLoc, BlockPos toLoc, PlayerMoveEvent moveEvent) {

		plugin.getCache(player).setLastBlockPos(toLoc);
	...
	*/
	...
	public void onPlayerChangePlotEvent(PlayerChangePlotEvent event) {
		if (!TownyUniverse.getInstance().hasResident(event.getPlayer().getUUID()))
			return;
	...
	public void onOutlawEnterTown(PlayerEnterTownEvent event) {
		...
		Resident outlaw = TownyUniverse.getInstance().getResident(event.getPlayer().getUUID());
		...
		if (town.hasOutlaw(outlaw))
			ResidentUtil.outlawEnteredTown(outlaw, town, event.getPlayer().getBlockPos());
	}
	...
	public void onPlayerDieInTown(PlayerDeathEvent event) {
		...
		TownBlock tb = TownyAPI.getInstance().getTownBlock(event.getEntity().getBlockPos());
	...
	public void onPlayerEnterTown(PlayerEnterTownEvent event) {
		...
		if (resident != null && resident.isSeeingBorderTitles() && town != null && TownySettings.isNotificationUsingTitles()) {
			String title = ChatFormatting.translateAlternateColorCodes('&', TownySettings.getNotificationTitlesTownTitle());
			String subtitle = ChatFormatting.translateAlternateColorCodes('&', TownySettings.getNotificationTitlesTownSubtitle());
	...
	public void onPlayerLeaveTown(PlayerLeaveTownEvent event) {
		Resident resident = TownyAPI.getInstance().getResident(event.getPlayer().getUUID());
		...
		if (TownySettings.isNotificationUsingTitles() && resident.isSeeingBorderTitles() && event.getTo().getTownBlockOrNull() == null) {
			String title = ChatFormatting.translateAlternateColorCodes('&', TownySettings.getNotificationTitlesWildTitle());
			String subtitle = ChatFormatting.translateAlternateColorCodes('&', TownySettings.getNotificationTitlesWildSubtitle());
	...
	public void onPlayerTakeLecternBookEvent(PlayerTakeLecternBookEvent event) {
		...
		//Make decision on whether this is allowed using the PlayerCache and then a cancellable event.
		event.setCancelled(!TownyActionEventExecutor.canDestroy(event.getPlayer(), event.getLectern().getBlockPos(), Material.LECTERN));
	}
	...
	public void onPlayerUsesCommand(PlayerCommandPreprocessEvent event) {
		...
		// BlockPos-dependent blocked commands
	...
	public void onEggLand(PlayerEggThrowEvent event) {
		if (TownySettings.isItemUseMaterial(Material.EGG, event.getEgg().getBlockPos()) && !TownyActionEventExecutor.canItemuse(event.getPlayer(), event.getEgg().getBlockPos(), Material.EGG))
