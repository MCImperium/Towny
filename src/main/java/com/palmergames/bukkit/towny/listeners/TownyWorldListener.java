...
public class TownyWorldListener implements Listener {
	...
	private void newWorld(Level world) {
	...
	public void onLightningStrike(LightningStrikeEvent event) {
		...
		if (event.getCause().equals(LightningStrikeEvent.Cause.TRIDENT)) {
			...
			townyWorld.addTridentStrike(event.getLightning().getUUID());
			
			Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, () -> townyWorld.removeTridentStrike(event.getLightning().getUUID()), 20L);
		}
	}
	...
	public void onStructureGrow(StructureGrowEvent event) {
		...
		// The event BlockPos is always one spot, and although 2x2 trees technically should have 4 locations, 
		// we can trust that the saplings were all placed by one person, or group of people, who were allowed
		// to place them.
		List<BlockState> disallowed = BorderUtil.disallowedBlocks(event.getBlocks(), event.getBlockPos().getBlock(), event.getPlayer());
	...
	public void onPortalCreate(PortalCreateEvent event) {
		...
		for (BlockState block : event.getBlocks()) {
			...
			if (!TownyActionEventExecutor.canBuild(player, block.getBlockPos(), Material.NETHER_PORTAL)) {
