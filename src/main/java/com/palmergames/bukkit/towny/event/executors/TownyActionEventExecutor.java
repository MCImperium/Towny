...
public class TownyActionEventExecutor {
	 ...
	 * @param loc   - BlockPos of the event.
	 ...
	 */
	private static boolean isAllowedAction(Player player, BlockPos loc, Material mat, ActionType action, TownyActionEvent event) {
	 ...
	 * 
	 * @param loc - BlockPos being tested.
	 * @return true if the explosion is allowed.
	 */
	private static boolean isAllowedExplosion(BlockPos loc) {
	...
	private static List<Block> filterExplodingBlockList(List<Block> blocks) {
		...
		for (Block block : blocks) {
			if (isAllowedExplosion(block.getBlockPos()))
				approvedBlocks.add(block);
		}
		return approvedBlocks;
	}
	
	private static Block getBlock(BlockPos loc) {
	...
	private static boolean isAllowedBurn(Block block) {
		 ...
		 */
			...
			&& (!townyWorld.isForceFire() && !TownyAPI.getInstance().getTownBlock(block.getBlockPos()).getPermissions().fire)) // Normal fire rules. 
	 ...
	 */
	public static boolean canBuild(Player player, BlockPos loc, Material mat) {
	...
	public static boolean canBuild(Player player, Block block) {
		return canBuild(player, block.getBlockPos(), block.getType());
	}
	 ...
	 * @return true if allowed.
	 */
	public static boolean canDestroy(Player player, BlockPos loc, Material mat) {
	...
	public static boolean canDestroy(Player player, Block block) {
		return canDestroy(player, block.getBlockPos(), block.getType());
	}
	 ...
	 * @return true if allowed.
	 */
	public static boolean canSwitch(Player player, BlockPos loc, Material mat) {
	 ...
	 * @return true if allowed.
	 */
	public static boolean canItemuse(Player player, BlockPos loc, Material mat) {
	 ...
	 * 
	 * @param loc - BlockPos to check
	 * @param harmedEntity - Entity which will be damaged.
	 * @param cause - DamageCause which has caused the damage.
	 * @return true if allowed.
	 */
	public static boolean canExplosionDamageEntities(BlockPos loc, Entity harmedEntity, DamageCause cause) {
		/*
	...
	public static boolean canBurn(Block block) {
		 ...
		 */
		return !BukkitTools.isEventCancelled(new TownyBurnEvent(block, block.getBlockPos(), TownyAPI.getInstance().getTownBlock(block.getBlockPos()), cancelled));
	}
}
