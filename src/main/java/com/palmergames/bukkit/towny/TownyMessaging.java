...
public class TownyMessaging {
	...
	public static void sendErrorMsg(Object sender, String msg) {
		...
		if (sender instanceof Player toSend) {
			sendMessage(toSend, Translatable.of("default_towny_prefix").stripColors(sender instanceof ConsolePlayer).append(Colors.Red + msg).forLocale(toSend));
	 ...
	 * Uses default_towny_prefix
	 *
	 * @param sender the Player receiving the msg
	 * @param msg the message being sent
	 */
	public static void sendMsg(Player sender, String msg) {
		if (sender == null || msg.isEmpty())
			return;
		
		if (sender instanceof Player p) {
			sendMessage(p, Translatable.of("default_towny_prefix").forLocale(p) + Colors.LightGreen + msg);
		} else if (sender instanceof ConsolePlayer) {
	 ...
	 */
	public static void sendTownBoard(Player sender, Town town) {
	 ...
	 */
	public static void sendNationBoard(Player sender, Nation nation) {
	...
	public static void sendRequestMessage(Player player, Invite invite) {
	 ...
	 * @param player - The player (Player) to send the confirmation
	 ...
	 */
	public static void sendInvitationMessage(Player player, String firstline, String confirmline, String cancelline) {
		final Translator translator = Translator.locale(player);
	 ...
	 * @param sender - The player (Player) to send the confirmation
	 * @param confirmation - Confirmation to send to the player.
	 */
	public static void sendConfirmationMessage(Player sender, Confirmation confirmation) {
	...
	public static void sendTownList(Player sender, List<TextComponent> towns, ComparatorType compType, int page, int total) {
	...
	public static void sendNationList(Player sender, List<TextComponent> nations, ComparatorType compType, int page, int total) {
		Translator translator = Translator.locale(sender);
	...
	public static void sendOutpostList(Player player, Town town, int page, int total) {
		...
		for (int i = (page - 1) * 10; i < iMax; i++) {
			BlockPos outpost = outposts.get(i);
	...
	public static void sendJailList(Player player, Town town, int page, int total) {
		...
		String headerMsg = ChatFormatting.GOLD + "# " +
							ChatFormatting.DARK_GRAY + "- "+
							ChatFormatting.GREEN + "Jail Name " +
							ChatFormatting.DARK_GRAY + "- "+
							ChatFormatting.BLUE + "Coord " +
							ChatFormatting.DARK_GRAY + "- " +
							ChatFormatting.YELLOW + "Cell Count " +
							ChatFormatting.DARK_GRAY + "- " +
							ChatFormatting.RED + "Primary Jail";
		for (int i = (page - 1) * 10; i < iMax; i++) {
			...
			TextComponent cellCount = Component.text(String.valueOf(jail.getJailCellBlockPoss().size()), NamedTextColor.YELLOW);
			TextComponent dash = Component.text(" - ", NamedTextColor.DARK_GRAY);
	...
	public static void sendPlotGroupList(Player sender, Town town, int page, int total) {
		Translator translator = Translator.locale(sender);
		...
		String headerMsg = ChatFormatting.GOLD + "# " +
				ChatFormatting.DARK_GRAY + "- "+
				ChatFormatting.GREEN + "Group Name " +
				ChatFormatting.DARK_GRAY + "- " +
				ChatFormatting.YELLOW + "Plot Size " +
				ChatFormatting.DARK_GRAY + "- " +
				ChatFormatting.BLUE + "For Sale";
		for (int i = (page - 1) * 10; i < iMax; i++) {
	 ...
	 *  
	 * @param sender Player who will see the message. 
	 * @param translatables Translatable object(s) which will be translated, joined with a space.
	 * @see #sendMsg(Player, Translatable)
	 */
	public static void sendMsg(Player sender, Translatable... translatables) {
	 ...
	 */
	public static void sendMsg(Player sender, Translatable translatable) {
	 ...
	 * @see #sendMessage(Player, Translatable)    
	 */
	public static void sendMessage(Player sender, Translatable... translatables) {
	 ...
	 * @param translatable Translatable object which will be translated.
	 */
	public static void sendMessage(Player sender, Translatable translatable) {
	 ...
	 * 
	 * @param sender Player who will receive the error message.
	 * @param translatables Translatable... object(s) to be translated using the locale of the end-user.
	 * @see #sendErrorMsg(Player, Translatable)    
	 */
	public static void sendErrorMsg(Player sender, Translatable... translatables) {
	 ...
	 */
	public static void sendErrorMsg(Player sender, Translatable translatable) {
	...
	public static void sendStatusScreen(Player sender, StatusScreen screen) {
	 ...
	 * 
	 * @deprecated since 0.97.3.0 use {@link #sendErrorMsg(Player, Translatable...)}
	 */
	@Deprecated 
	public static void sendErrorMsg(Object sender, String[] msg) {
		for (String line : msg) {
